# Usage: ./SubCommandDemo.exe --completion-file-pwsh > myapp-completion.ps1
# Then in PowerShell:
...
#   . ./myapp-completion.ps1
# To make it permanent, add the above line to your $PROFILE
# PowerShell argument completer for SubCommandDemo.exe
...
$scriptBlock = {
  ...
  $out = & "./SubCommandDemo.exe" __complete @argsList 2>$null
  if (-not $out) { return @() }
...
Register-ArgumentCompleter -CommandName "SubCommandDemo.exe" -ScriptBlock $scriptBlock
Register-ArgumentCompleter -CommandName "SubCommandDemo" -ScriptBlock $scriptBlock
Register-ArgumentCompleter -CommandName "./SubCommandDemo.exe" -ScriptBlock $scriptBlock
Register-ArgumentCompleter -CommandName ".\SubCommandDemo.exe" -ScriptBlock $scriptBlock
Register-ArgumentCompleter -CommandName ".\\SubCommandDemo.exe" -ScriptBlock $scriptBlock
...
if ($PSVersionTable.PSVersion.Major -ge 7) {
  Register-ArgumentCompleter -Native -CommandName "SubCommandDemo" -ScriptBlock {
    param($wordToComplete, $commandAst, $cursorPosition)
